...
@push('scripts')
    <script type="text/x-template" id="logo-template">
        <a
            :class="`left position-static w-auto ${addClass}`"
            href="{{ route('shop.home.index') }}"
            aria-label="Logo">

            @if ($logo = core()->getCurrentChannel()->logo_url)
                <img class="logo img-fluid" src="{{ $logo }}" alt="logo {{ json_decode(core()->getCurrentChannel()->home_seo)->meta_title }}"/>
            @else
                <img class="logo" src="{{ asset('themes/velocity/assets/images/logo-text.png') }}" alt="logo {{ json_decode(core()->getCurrentChannel()->home_seo)->meta_title }}" width="200" height="50" />
            @endif
        </a>
    </script>
